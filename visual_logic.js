...
this.procedures["ALL_STEPS"] = ALL_STEPS;
this.procedures["STEP"] = STEP;
this.procedures["disable_all"] = disable_all;
this.procedures["outline"] = outline;
this.procedures["lms report progress"] = lms_report_progress;
this.procedures["button_scale"] = button_scale;
this.procedures["marker_updt"] = marker_updt;

var cooler_animated, input_range_start, input_range_end, result_range_start, result_range_end, input_value, animation_stage, STEP_FRAME, MSG, STEP_MSG, MSG_VOICE, HIGHLIGHT, DO, SOUND, all_sounds, object_to_outline, progress, current_step, result_range, output_value, bar_raycast, prev_step, timeline_drag, animation_paused, timeline_marker_hovered, scale_multiplier, count, button_x_position, CURRENT_FRAME, play;
...
// Describe this function...
function ALL_STEPS(animation_stage) {
  if (current_step == 0) {
    STEP(0, 'click Play to start learning', 'Unboxing', loadMedia_WebAudio('./sounds/step_0_voice.mp3'), 'box', animation_stage, null);
  } else if (current_step == 1) {
    STEP(49, 'take out the parts from the box', 'STEP 1', loadMedia_WebAudio('./sounds/step_1_voice.mp3'), ['inner_bottom_fit_box', 'inner_top_fit_box'], animation_stage, loadMedia_WebAudio('./sounds/stage_1.mp3'));
  } else if (current_step == 2) {
    STEP(99, 'lay out the parts on the desk', 'STEP 2', loadMedia_WebAudio('./sounds/step_2_voice.mp3'), ['GROUP', 'animated_objects'], animation_stage, loadMedia_WebAudio('./sounds/stage_2.mp3'));
  } else if (current_step == 3) {
    STEP(149, 'assemble the frame', 'STEP 3', loadMedia_WebAudio('./sounds/step_3_voice.mp3'), ['cooler_leg_1', 'cooler_leg_2', 'thrust_1', 'thrust_2', 'thrust_3', 'bolt_round_m6_1', 'bolt_round_m6_2', 'bolt_round_m6_3', 'bolt_round_m6_4', 'bolt_round_m6_5', 'bolt_round_m6_6'], animation_stage, loadMedia_WebAudio('./sounds/stage_3.mp3'));
  } else if (current_step == 4) {
    STEP(199, 'fasten the casing to the frame', 'STEP 4', loadMedia_WebAudio('./sounds/step_4_voice.mp3'), ['cooler_holder', 'handle_m6_4', 'handle_m6_6', 'bolt_m6_1', 'bolt_m6_2'], animation_stage, loadMedia_WebAudio('./sounds/stage_4.mp3'));
  } else if (current_step == 5) {
    STEP(249, 'insert batteries in the motor', 'STEP 5', loadMedia_WebAudio('./sounds/step_5_voice.mp3'), ['cooler_base', 'battery_1130_1', 'battery_1130_2', 'cooler_battery_cap'], animation_stage, loadMedia_WebAudio('./sounds/stage_5.mp3'));
  } else if (current_step == 6) {
    STEP(299, 'install the motor and the fan', 'STEP 6', loadMedia_WebAudio('./sounds/step_6_voice.mp3'), ['cooler_base', 'cooler'], animation_stage, loadMedia_WebAudio('./sounds/stage_6.mp3'));
  } else if (current_step == 7) {
    STEP(349, 'attach the back cover', 'STEP 7', loadMedia_WebAudio('./sounds/step_7_voice.mp3'), ['cooler_concentrator', 'screw_m3_1', 'screw_m3_2', 'screw_m3_3'], animation_stage, loadMedia_WebAudio('./sounds/stage_7.mp3'));
  } else if (current_step == 8) {
    STEP(399, 'install the front cover', 'STEP 8', loadMedia_WebAudio('./sounds/step_8_voice.mp3'), 'cooler_cap', animation_stage, loadMedia_WebAudio('./sounds/stage_8.mp3'));
  } else if (current_step == 9) {
    STEP(449, 'position the cooler in desired direction and tighten the nuts', 'STEP 9', loadMedia_WebAudio('./sounds/step_9_voice.mp3'), ['cooler_holder', 'cooler_cap', 'cooler_concentrator', 'handle_m6_4', 'handle_m6_6'], animation_stage, loadMedia_WebAudio('./sounds/stage_9.mp3'));
  } else if (current_step == 10) {
    STEP(499, 'Enjoy your fresh breeze!', 'Done!', loadMedia_WebAudio('./sounds/step_10_voice.mp3'), ['GROUP', 'animated_objects'], animation_stage, loadMedia_WebAudio('./sounds/stage_10.mp3'));
  }
}
...
function STEP(STEP_FRAME, MSG, STEP_MSG, MSG_VOICE, HIGHLIGHT, DO, SOUND) {
  if (DO == 'play') {
    disable_all();
    count = 0;
    ...
    operateAnimation('PLAY', getAnimations(['GROUP', 'animated_objects']), CURRENT_FRAME, STEP_FRAME, 'LoopOnce', 0.5,
            function() {
      count = (typeof count === 'number' ? count : 0) + 1;
      if (count <= 1) {
        updateTextObj('text_main', MSG);
        updateTextObj('text_step', STEP_MSG);
        playSound(MSG_VOICE, false);
        CURRENT_FRAME = STEP_FRAME;
        outline(HIGHLIGHT);
        if (current_step == 10) {
          changeVis('button_fan', true);
          registerSetTimeout(0, function() {
            changeVis('button_play', false);
            changeVis('button_pause', false);
          });
        } else {
          changeVis('button_play', true);
          registerSetTimeout(0, function() {
            changeVis('button_fan', false);
            changeVis('button_pause', false);
          });
        }
        play = false;
        lms_report_progress(current_step * 10);
      }
    }, false);

        } else if (DO == 'pause') {

    operateAnimation('PAUSE', getAnimations(['GROUP', 'animated_objects']), null, null, 'AUTO', 1,
            function() {}, false);

        animation_paused = true;
    cooler_animated = false;
    stopSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'));
    pauseSound(SOUND);
  } else if (DO == 'resume') {

    operateAnimation('RESUME', getAnimations(['GROUP', 'animated_objects']), null, null, 'AUTO', 1,
            function() {}, false);
    ...
    operateAnimation('SET_FRAME', getAnimations(['GROUP', 'animated_objects']), STEP_FRAME, null, 'AUTO', 1,
            function() {}, false);
    ...
    updateTextObj('text_main', MSG);
    updateTextObj('text_step', STEP_MSG);
    if (current_step == 10 && CURRENT_FRAME >= 499) {
      changeVis('button_fan', true);
      registerSetTimeout(0, function() {
        changeVis('button_pause', false);
        changeVis('button_play', false);
      ...
      registerSetTimeout(0, function() {
        ...
        cooler_animated = false;
        stopSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'));
        rewindSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'));
      });
    }
  }
}
...
// outline puzzle
function outline2(objSelector, doWhat) {
    var objNames = retrieveObjectNames(objSelector);
...
function disable_all() {
  ...
  operateAnimation('STOP', getAnimations(['GROUP', 'animated_objects']), null, null, 'AUTO', 1,
          function() {}, false);

      outline2(['ALL_OBJECTS'], 'DISABLE');
...
function initGetCameraDirection() {
    ...
    bindListener(elem, 'mousemove', coordsCallback);
    bindListener(elem, 'mousedown', coordsCallback);
    bindListener(elem, 'mouseup', coordsCallback);
    bindListener(elem, 'touchstart', coordsCallback);
    bindListener(elem, 'touchend', coordsCallback);
};
...
function raycast(fromPosObj, dir, intersectObjsSelector) {
    ...
    if (objs.length) {
        ...
        var intersects = _pGlob.raycasterTmp.intersectObjects(objs);
...
// Describe this function...
function outline(object_to_outline) {
  outline2(object_to_outline, 'ENABLE');
  playSound(loadMedia_WebAudio('./sounds/flicker.mp3'), true);
  registerSetTimeout(2.8, function() {
    outline2(object_to_outline, 'DISABLE');
    stopSound(loadMedia_WebAudio('./sounds/flicker.mp3'));
  });
}
...
// Describe this function...
function lms_report_progress(progress) {
  LMSSetValue(dataModelElements('cmi.core.score.raw'), progress);if (progress == 100) {
...
function button_scale() {
  if (getHTMLElemAttribute('innerWidth', ['WINDOW'], false) < 800) {
    scale_multiplier = map_range(800, 0, 1, 6, getHTMLElemAttribute('innerWidth', ['WINDOW'], false));
    button_x_position = map_range(800, 0, 0.011, 0.07, getHTMLElemAttribute('innerWidth', ['WINDOW'], false));
    setObjTransform('button_play', false, 'scale', [scale_multiplier, scale_multiplier, scale_multiplier], false);
    setObjTransform('button_next_step', false, 'scale', [scale_multiplier, scale_multiplier, scale_multiplier], false);
    setObjTransform('button_previous_step', false, 'scale', [scale_multiplier, scale_multiplier, scale_multiplier], false);
    setObjTransform('button_pause', false, 'scale', [scale_multiplier, scale_multiplier, scale_multiplier], false);
    setObjTransform('button_fan', false, 'scale', [scale_multiplier, scale_multiplier, scale_multiplier], false);
    setObjTransform('button_previous_step', false, 'position', [button_x_position * -1, '', ''], false);
    setObjTransform('button_next_step', false, 'position', [button_x_position, '', ''], false);
  } else {
    setObjTransform('button_play', false, 'scale', [1, 1, 1], false);
    setObjTransform('button_fan', false, 'scale', [1, 1, 1], false);
    setObjTransform('button_next_step', false, 'scale', [1, 1, 1], false);
    setObjTransform('button_pause', false, 'scale', [1, 1, 1], false);
    setObjTransform('button_previous_step', false, 'scale', [1, 1, 1], false);
    setObjTransform('button_previous_step', false, 'position', [-0.011, '', ''], false);
    setObjTransform('button_next_step', false, 'position', [0.011, '', ''], false);
  }
}
...
// Describe this function...
function marker_updt() {
  setObjTransform('timeline_marker', false, 'position', [map_range(0, 499, -0.135, 0.135, getAnimationFrame('cooler_holder')), '', ''], false);
}


registerOnClick('button_next_step', false, false, [0,1,2], function() {
  if (current_step < 10) {
    if (animation_paused == false && !isAnimationPlaying(getAnimations(['GROUP', 'animated_objects']))) {
...
registerOnClick('button_play', false, false, [0,1,2], function() {
  if (!isAnimationPlaying(getAnimations(['GROUP', 'animated_objects']))) {
  ...
  changeVis('button_play', false);
  registerSetTimeout(0, function() {
    changeVis('button_pause', true);
  });
}, function() {});

volume(loadMedia_WebAudio('./sounds/step_0_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_1_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_2_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_3_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_4_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_5_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_6_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_7_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_8_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_9_voice.mp3'), 1);
volume(loadMedia_WebAudio('./sounds/step_10_voice.mp3'), 1);

volume(loadMedia_HTML5('./sounds/environment_music.mp3'), 0.05);
volume(loadMedia_WebAudio('./sounds/flicker.mp3'), 0.2);
volume(loadMedia_WebAudio('./sounds/stage_1.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_2.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_3.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_4.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_5.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_6.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_7.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_8.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/stage_9.mp3'), 0.6);
volume(loadMedia_WebAudio('./sounds/cooler_working.mp3'), 1);

registerOnClick('button_previous_step', false, false, [0,1,2], function() {
  if (current_step > 0) {
    current_step = current_step - 1;
  }
  ALL_STEPS('set');
...
registerOnClick('button_pause', false, false, [0,1,2], function() {
  if (animation_paused == false && (isAnimationPlaying(getAnimations(['GROUP', 'animated_objects'])) || cooler_animated == true)) {
    ALL_STEPS('pause');
    stopSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'));
    rewindSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'));
    if (current_step == 10 && CURRENT_FRAME >= 499) {
...
checkPerformance(function() {
  ...
  setLightParam('USE_SHADOW', getObjectsFrom(['ALL_OBJECTS'], 'LIGHT'), false);
});

all_sounds = [loadMedia_WebAudio('./sounds/flicker.mp3'), loadMedia_WebAudio('./sounds/stage_1.mp3'), loadMedia_WebAudio('./sounds/stage_2.mp3'), loadMedia_WebAudio('./sounds/stage_3.mp3'), loadMedia_WebAudio('./sounds/stage_4.mp3'), loadMedia_WebAudio('./sounds/stage_5.mp3'), loadMedia_WebAudio('./sounds/stage_6.mp3'), loadMedia_WebAudio('./sounds/stage_7.mp3'), loadMedia_WebAudio('./sounds/stage_8.mp3'), loadMedia_WebAudio('./sounds/stage_9.mp3'), loadMedia_WebAudio('./sounds/cooler_working.mp3'), loadMedia_WebAudio('./sounds/step_0_voice.mp3'), loadMedia_WebAudio('./sounds/step_1_voice.mp3'), loadMedia_WebAudio('./sounds/step_2_voice.mp3'), loadMedia_WebAudio('./sounds/step_3_voice.mp3'), loadMedia_WebAudio('./sounds/step_4_voice.mp3'), loadMedia_WebAudio('./sounds/step_5_voice.mp3'), loadMedia_WebAudio('./sounds/step_6_voice.mp3'), loadMedia_WebAudio('./sounds/step_7_voice.mp3'), loadMedia_WebAudio('./sounds/step_8_voice.mp3'), loadMedia_WebAudio('./sounds/step_9_voice.mp3'), loadMedia_WebAudio('./sounds/step_10_voice.mp3')];

registerOnClick('timeline', false, false, [0,1,2], function() {
  bar_raycast = raycast(getActiveCamera(), getCameraDirection(true, false), 'timeline')[0];
  setObjTransform('timeline_marker', false, 'position', [map_range(0, 1, -0.135, 0.135, dictGet(bar_raycast, 'uv')[0]), '', ''], false);

  operateAnimation('SET_FRAME', getAnimations(['GROUP', 'animated_objects']), map_range(0, 1, 0, 499, dictGet(bar_raycast, 'uv')[0]), null, 'AUTO', 1,
          function() {}, false);

      current_step = Math.floor(map_range(0, 1, 0, 10, dictGet(bar_raycast, 'uv')[0]));
  CURRENT_FRAME = map_range(0, 1, 0, 499, dictGet(bar_raycast, 'uv')[0]);
  ALL_STEPS('marker');
  animation_paused = false;
  cooler_animated = false;
}, function() {});

registerOnClick('button_fan', false, false, [0,1,2], function() {
  changeVis('button_fan', false);
  registerSetTimeout(0, function() {
    changeVis('button_pause', true);
  });
  playSound(loadMedia_WebAudio('./sounds/cooler_working.mp3'), false);
  cooler_animated = true;
  animation_paused = false;

  animateParam(4, 30, 16.4, 'Linear', 'InOut', 0, false, 'iJkn)QwKphvRL.zAG[hq',
      function() {
    applyObjLocalTransform('cooler', 'rotation', ['', _pGlob.animateParamUpdate || 0, '']);
    if (cooler_animated == false) {
      if (_pGlob.animateParamTweenInstance && _pGlob.animateParamTweenInstance['iJkn)QwKphvRL.zAG[hq']) _pGlob.animateParamTweenInstance['iJkn)QwKphvRL.zAG[hq'].stop()}
  },
      function() {
    changeVis('button_pause', false);
    registerSetTimeout(0, function() {
...
registerOnHover(['button_next_step', 'button_pause', 'button_play', 'button_previous_step', 'button_fan'], false, function() {
  setHTMLElemStyle('cursor', 'pointer', ['CONTAINER'], false);
}, function() {
  setHTMLElemStyle('cursor', 'default', ['CONTAINER'], false);
});

registerOnDrag('timeline_marker', false, [0,1,2], function() {
  prev_step = current_step;
  animation_paused = false;
}, function() {
  setHTMLElemStyle('cursor', 'grabbing', ['CONTAINER'], false);
  timeline_drag = true;
  dragMove('timeline_marker', 'X', 's76%pmkHae7bHuNUlWo^', '_8-?eAX[Roz]S:DQ}NRr');

  operateAnimation('SET_FRAME', getAnimations(['GROUP', 'animated_objects']), map_range(-0.135, 0.135, 0, 499, getObjTransform('timeline_marker', false, 'position', 'x')), null, 'AUTO', 1,
          function() {}, false);

      current_step = Math.floor(map_range(-0.135, 0.135, 0, 10, getObjTransform('timeline_marker', false, 'position', 'x')));
  CURRENT_FRAME = map_range(-0.135, 0.135, 0, 499, getObjTransform('timeline_marker', false, 'position', 'x'));
  ...
  timeline_drag = false;
  if (timeline_marker_hovered == true) {
    setHTMLElemStyle('cursor', 'grab', ['CONTAINER'], false);
  } else {
    setHTMLElemStyle('cursor', 'default', ['CONTAINER'], false);
  }
}, '_8-?eAX[Roz]S:DQ}NRr');
...
eventHTMLElem('click', ['CONTAINER'], false, function(event) {
  if (!isSoundPlaying(loadMedia_HTML5('./sounds/environment_music.mp3'))) {
    playSound(loadMedia_HTML5('./sounds/environment_music.mp3'), true);
    if (current_step == 0) {
      playSound(loadMedia_WebAudio('./sounds/step_0_voice.mp3'), false);
    }
  }
});

registerOnHover('timeline_marker', false, function() {
  if (timeline_drag == false) {
    setHTMLElemStyle('cursor', 'grab', ['CONTAINER'], false);
  }
  timeline_marker_hovered = true;
}, function() {
  setHTMLElemStyle('cursor', 'default', ['CONTAINER'], false);
  timeline_marker_hovered = false;
});
...
registerEveryFrame(function() {
  if (timeline_drag == false && play == true) {
    marker_updt();
  }
});
